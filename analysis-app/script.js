...
function loadImportantFiles() {
  ...
  if (fs.existsSync("./resources/default-pit.json")) {
    defaultPitJSON = JSON.parse(fs.readFileSync("./resources/default-pit.json"));
  }
  if (fs.existsSync("./resources/default-stand.json")) {
    defaultStandJSON = JSON.parse(fs.readFileSync("./resources/default-stand.json"));
  }
...
apiKey.apiKey = fs.readFileSync("./resources/tba-api-key.txt").toString();
...
// game specific
const table_values = ["Cross", "Hatch", "Cargo", "Stop"];
...
function addStandDataToPage() {
  ...
  for (let match_id in stand_data[selected_team]) {
    ...
    // adds notes to row
    append_html += `<td class="notes-td" id="notes-` + selected_team + `-` + match["info"]["match"] + `">` + match["Notes"]["Notes"] + `</td>`;
    ...
    for (let btn in button_values_keys) {
      ...
      switch (btn_title) {
        ...
        // defense button
        case "defense":
          // adds data to defense button
          addButtonData(btn_title, match["info"]["match"], [
            function() { return match["info"]["match"]; },
            function() { return match["Teleop"]["Played Defense"]; },
            function() { return match["Teleop"]["Faced Defense"]; }
          ]);
      }
    }
...
function addButtonData(code, match, obtainValues) {
  ...
  let added_classes = "";
  // gives notes a notes-td class
  if (obtainValues[0]() == "Notes") {
    added_classes = "notes-td";
  }
...
function createMatch(loc, match_number, team) {
  ...
  $(".predict-" + match_number).click(function() {
    ...
    for (let team_index in schedule[match_number.toString()]) {
      ...
      // a list of scores
      let scores = calculateScores(team)[0];
      // mean score
...
function displaySensitiveInfo() {
  if (sensitive_info) {
    ...
    $(".notes-td").css("visibility", "hidden");
  } else {
    $(".toggle-sensitive-info").text("Hide Sensitive Info");
    $(".notes-td").css("visibility", "visible");
  }
}
